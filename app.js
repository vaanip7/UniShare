const state = {
  ...
  schedule: [
    { time: '9:00 – 10:00', subject: 'Data Structures', prof: '', room: 'Room 301', color: '#378ADD' },
...
const pages = {
  dashboard: () => ({
    title: `Good morning, ${state.user.name.split(' ')[0]}`,
    ...
    html: `
      ...
      <div class="content-grid">
        <div class="card">
          <div class="card-header"><span class="card-title">Pending Assignments</span><a class="card-link" onclick="nav('assignments')">View all</a></div>
          ${state.assignments.slice(0,4).map(assignmentRow).join('')}
        </div>
        <div class="card">
          <div class="card-header"><span class="card-title">Recent Study Notes</span><a class="card-link" onclick="nav('notes')">View all</a></div>
          ${state.notes.map(noteRow).join('')}
        </div>
      </div>
      <div class="card">
        <div class="card-header"><span class="card-title">Today's Schedule</span><a class="card-link" onclick="nav('schedule')">Full timetable</a></div>
        ${state.schedule.map(scheduleRow).join('')}
      </div>`
  }),
  assignments: () => ({
    title: 'Assignments', sub: 'Track your pending and submitted work',
    html: `<div class="card"><div class="card-header"><span class="card-title">All Assignments</span></div>${state.assignments.map(assignmentRow).join('')}</div>`
  }),
  notes: () => ({
    title: 'Study Notes', sub: 'Download materials uploaded by your faculty',
    html: `<div class="card"><div class="card-header"><span class="card-title">All Study Notes</span></div>${state.notes.map(noteRow).join('')}</div>`
  }),
  schedule: () => ({
    title: 'Timetable', sub: 'Your weekly class schedule',
    html: `<div class="card"><div class="card-header"><span class="card-title">Weekly Timetable</span></div>${state.schedule.map(scheduleRow).join('')}</div>`
  }),
  scores: () => ({
    title: 'My Scores', sub: 'Performance across all subjects',
    html: `<div class="card"><div class="card-header"><span class="card-title">My Scores</span></div>${state.scores.map(s => `
      <div class="progress-row">
        <span class="progress-label">${s.sub}</span>
        <div class="progress-track"><div class="progress-fill" style="width:${s.score}%;background:${s.color}"></div></div>
        <span class="progress-value">${s.score}%</span>
      </div>`).join('')}</div>`
  }),
  submit: () => ({
    title: 'Submit Work', sub: 'Upload your assignment files here',
    html: `
      <div class="card" style="max-width:520px">
        ...
        <div class="upload-area" onclick="document.getElementById('file-input').click()">
          <div class="list-icon icon-note" style="margin:0 auto">
            <svg width="15" height="15" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.3"><path d="M8 2v8M5 7l3 3 3-3" stroke-linecap="round" stroke-linejoin="round"/><path d="M2 12h12" stroke-linecap="round"/></svg>
          </div>
          <p>Click to upload your file</p>
          <span style="font-size:11px;color:#999">PDF, DOCX up to 10 MB</span>
          <input type="file" id="file-input" style="display:none" accept=".pdf,.docx" onchange="alert('File selected: '+this.value)">
        </div>
        <button class="submit-btn" onclick="alert('Assignment Submitted!')">Submit Assignment</button>
      </div>`
  })
};
...
function openSettings() {
  ...
  document.querySelectorAll('.color-opt').forEach(o => o.classList.toggle('active', o.dataset.color === state.user.accent));
...
function updateUser() {
  ...
  document.getElementById('user-display-program').innerText = state.user.program;
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (saved) { Object.assign(state.user, JSON.parse(saved)); applyAccent(state.user.accent); }
  updateUser();
  nav('dashboard');
  document.querySelectorAll('.nav-item').forEach(b => b.addEventListener('click', () => nav(b.dataset.section)));
  document.querySelectorAll('.color-opt').forEach(o => o.addEventListener('click', function() {
